...
bool bfs_wrestler(node *Q1,int s)
{
    for(int i=1;i<=n;i++)
    {
        visited[i]=-1;
    ...
    while(Q1->next!=NULL)
    {
        ...
        for(int i=1;i<=n;i++)
        {
            ...
            else if(a[u][i]==1&&visited[i]==visited[u])
            {
                return false;
            }
        }
...
int main()
{
    ...
    else
    {
        ...
        display();
        BFS_display();
        bool m=bfs_wrestler(first1,1);
        if(m==true)
        {
            printf("\nThe designation in the professional wrestlers are possible\n");
            for(i=1;i<=n;i++)
            {
                if(visited[i]==1)
                {
                    printf("\nWrestler No.%d is in the babyfaces\n",i);
                }
                else
                {
                    printf("\nWrestler No.%d is in the heels\n",i);\
                }
            }
        }
        else
        {
            printf("\nThe designation between the professional wrestlers are not possible\n");
        }
    }
    return 0;
}
